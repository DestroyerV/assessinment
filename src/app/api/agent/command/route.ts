...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 1. Fetch current state context (names only to save tokens)
    ...
    try {
        ...
        return NextResponse.json({ error: 'Failed to interpret command' }, { status: 400 });
    }
    ...
    for (const action of actions) {
        try {
            if (action.type === 'CREATE_ROLE') {
                // Check exist
                const exist = await prisma.role.findUnique({ where: { name: action.name } });
                if (!exist) {
                 ...
                 const exist = await prisma.permission.findUnique({ where: { name: action.name } });
                 if (!exist) {
                ...
                // Find IDs
                const role = await prisma.role.findUnique({ where: { name: action.role } });
                let permission = await prisma.permission.findUnique({ where: { name: action.permission } });
                ...
                if (!permission) {
                     // Auto-create permission if missing during assignment? 
                     // The prompt instructions say "If user asks to assign... create it first", so LLM should have emitted a CREATE_PERMISSION action before this.
                     // But just in case, let's create it if LLM missed it.
                     permission = await prisma.permission.create({
                         data: { name: action.permission, description: "Auto-created by agent" }
                     });
                ...
                if (!exists) {
                    ...
                    results.push(`'${permission.name}' is already assigned to '${role.name}'`);
                }
            }
        } catch (err: any) {
